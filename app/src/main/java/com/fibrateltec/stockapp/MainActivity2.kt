...
//object en Kotlin define un singleton, lo que significa que EmpleadosManager tendrá una única instancia en toda la aplicación. Esto es útil para mantener un estado global o compartir datos entre diferentes partes de la aplicación.
object EmpleadosManager {
    //es una variable mutable que contiene una lista de objetos Empleado.
    var empleadosList = mutableListOf<Empleado>()
    //es una variable de tipo EmpleadoAdapter que se declara con lateinit.
...
class MainActivity2 : AppCompatActivity() {
        ...
        guardar.setOnClickListener {
            if (spinner.selectedItem != null) { // Verificar si hay un elemento seleccionado en el spinner
                ...
                if (cedula.text.toString().trim().isEmpty() || nombre.text.toString().trim().isEmpty() ||
                    ...
                    ejecutarServicio(
                        "https://fibrateltecsas.com/insertar_empleados.php",
    ...
    private fun obtenerIdUsuarios() {
        val url =
            "https://fibrateltecsas.com/verificar.php" // URL del archivo PHP para obtener ID de usuarios
